...
public class FindArchivedTravelsUseCase {
  ...
  public Page<UpcomingTravelCard> findArchivedTravels(final String username, final int page, final int limit) {
    final var archivedTravels = travelRepository.findArchivedTravels(username, LocalDate.now(), PageRequest.of(page, limit));
    if ( archivedTravels.hasContent() ) {
      final var participants = archivedTravels.stream()
          .map(Travel::getParticipants)
          .flatMap(Collection::stream)
          .map(Participant::getAccountId)
          .collect(Collectors.toSet());
      final var authors = archivedTravels.stream()
      ...
      return archivedTravels.map(it -> new UpcomingTravelCard(
          it.getId(),
          it.getStartDate(),
          it.getEndDate(),
          it.getDays().size(),
